...
class GPAW(PAW):
    ...
    # FOR QM/MM
    def get_point_charge_forces(self, mm_subsystem=None, all_pbc=None):
        """ Calculate the forces on point charges due to
            the external field set up by an electron
            density """
        ...
        if all_pbc == 2 or all_pbc is None:

            Forces = np.zeros((len(mm_subsystem), 3))

            q = mm_subsystem.get_initial_charges()

            ## print 'Here just before MIC forces:', str(datetime.now())

            for a in range(len(mm_subsystem)):
                Force = np.zeros(3)
                _gpaw.pc_der_potential(Force, self.density.nt_sG.sum(axis=0),
                                   mm_subsystem[a].position, q[a]*np.ones(2),
                                   beg_c, end_c, h)
            ...
            Forces = np.zeros((len(mm_subsystem), 3))
            q = mm_subsystem.get_initial_charges()
            C = mm_subsystem.cell.diagonal()

            for a in range(len(mm_subsystem)):
                Force = np.zeros(3)
                _gpaw.wolf_der_potential(Force, self.density.nt_sG.sum(axis=0),
                                         mm_subsystem[a].position, q[a]*np.ones(2),
                                         beg_c, end_c, h, C)
    ...
    def get_compensation_charges(self, index):
        ...
        charge should integrate to zero. """
